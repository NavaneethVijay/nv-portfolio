...
export function Experience() {
  ...
  return (
    <div className="container mx-auto px-0 md:px-0 py-20 md:py-28">
      <div className="bg-panel-ink text-panel-ink-fg p-1 md:p-14">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-x-12 gap-y-9 md:gap-y-0 items-end mb-14 md:mb-16">
          <h2 className="font-display text-[clamp(3.625rem,8vw,7.25rem)] font-medium tracking-[-0.03em] leading-[0.86]">
            Think in
            <br />
            <em className="font-serif italic text-brand-on-dark">systems.</em>
          </h2>
          <p className="text-[15px] leading-[1.55] text-panel-ink-fg/70 max-w-[330px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-px bg-panel-ink-fg/20 border-t border-panel-ink-fg/20">
          {Object.entries(skillCategories).map(([key, { title, skills }], i) => (
            <div key={key} className="bg-panel-ink px-4 md:px-[22px] py-6 min-h-[200px] md:border-r border-panel-ink-fg/20">
              <span className="text-[14px] text-brand-on-dark">
                {String(i + 1).padStart(2, "0")}
              </span>
              <h3 className="font-serif text-2xl my-7 tracking-[-0.03em]">{title}</h3>
